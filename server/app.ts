...
import { inNumberArray, isBetween, isRequiredAllOrNone, validateRequest } from './validations'
...
app.post('/', (req: Request, res: Response): any => {
    ...
    return res.status(200).json({ signature: sdkJWT })
})

app.listen(port, () => console.log(`BeanPal backend is ready, listening on port ${port}!`))
